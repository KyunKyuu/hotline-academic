    ...
    <div class="grid grid-2">
        <div class="card section stack" style="gap: 16px;">
            ...
            <div class="detail-list">
                ...
                <div style="margin-bottom: 12px; font-size:14px;"><strong class="muted" style="font-size:11px; text-transform:uppercase; letter-spacing:0.88px; display:block;">Referral</strong>{{ $contact->referral_code ? ($contact->referralCode ? $contact->referral_code . ' (' . $contact->referralCode->name . ')' : $contact->referral_code) : '-' }}</div>
                <div style="margin-bottom: 12px; font-size:14px;"><strong class="muted" style="font-size:11px; text-transform:uppercase; letter-spacing:0.88px; display:block;">Group</strong><span class="pill">Group {{ $contact->group_type ?: '-' }}@if($contact->group_type === 'A' && $contact->referralCode) ({{ $contact->referralCode->name }})@endif</span></div>
